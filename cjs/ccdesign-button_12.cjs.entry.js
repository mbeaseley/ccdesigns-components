...
const regexFormatter = require('./regexFormatter-909648c4.js');
...
const CcdesignButton = class {
    ...
    render() {
        ...
        const icon = this.icon !== undefined ? (index.h("ccdesign-icon", { name: this.icon, color: this.color, size: 'xs' })) : null;
        return (
        // @ts-ignore
        index.h(TagType, Object.assign({ class: classList }, attribute, { "aria-label": this.alt }), icon, index.h("span", null, index.h("slot", null))));
    }
...
const CcdesignCarouselNew = class {
    ...
    getData(carouseData) {
        ...
        // @ts-ignore
        index.h("ccdesign-lazy-image", { "img-src": item.image, classNames: 'slide-right', alt: item.alt })));
        return returnItems;
    }
    /**
    ...
    componentWillLoad() {
        if (this.data) {
            const formattedData = regexFormatter.regexFormatter(this.data, /(\bid|\bimage|\balt+?):/g);
     ...
     */
    render() {
        return index.h("div", { class: 'carousel' }, this.data ? this.dataElement : null);
    }
    get el() { return index.getElement(this); }
};
...
const CcdesignCopy = class {
     ...
     */
    render() {
        return index.h("div", { class: 'copy', innerHTML: this.copy });
    }
};
...
const CcdesignCvTemplate = class {
     ...
     */
    render() {
        const header = this.header ? index.h("ccdesign-header", { data: `${this.header}` }) : null;
        const footer = this.footer ? index.h("ccdesign-footer", { data: `${this.footer}` }) : null;
        return (index.h("div", null, header, index.h("main", { class: 'page' }, index.h("content", { class: 'page__body' }, index.h("slot", null), index.h("slot", { name: 'container' }))), footer));
    }
};
...
const CcdesignFooter = class {
    ...
    componentWillLoad() {
        if (this.data) {
            this.dataContent = regexFormatter.regexFormatter(this.data, /(\bid|\btext|\bbackgroundColor+?):/g);
        }
    }
    /**
     * render
    ...
    render() {
        if (this.data) {
            return (index.h("div", { class: `footer footer--${this.dataContent.backgroundColor}` }, `${this.dataContent.text}`));
        }
    }
};
...
const CcdesignHeader = class {
    constructor(hostRef) {
        ...
        this.isClient = index.getContext(this, "isClient");
    }
    /**
     * Formats nav data
     */
    formatContent() {
        if (this.data !== undefined) {
            this.formattedData = regexFormatter.regexFormatter(this.data, /([a-z]+?):/g);
        }
    }
    /**
     ...
     */
    getNav(data) {
        return (index.h("ul", { class: `navbar__list` }, index.h("li", null, index.h("a", { href: this.env }, index.h("ccdesign-lazy-image", { "img-src": 'assets/favicon.svg', alt: 'CCDesigns' }))), this.isRootPage ? (index.h("ccdesign-button", { icon: 'chevron-left', type: 'text', color: 'light-grey',
            // eslint-disable-next-line react/jsx-no-bind
            onClick: () => this.backRootPage(), alt: 'chevron-left' })) : (data.map((item) => (index.h("li", { class: `navbar__item` }, index.h("a", { id: 'desktop' + item.id, href: item.url, class: `navbar__link` }, item.name)))))));
    }
    /**
     * render
     */
    render() {
        const mobileNav = this.formattedData ? (index.h("mobile-nav", { "aria-label": 'Main Navigation', data: this.formattedData })) : null;
        const desktopNav = this.formattedData ? (index.h("nav", { class: 'navbar', "aria-label": 'Main Navigation' }, this.getNav(this.formattedData))) : null;
        return index.h("header", { class: 'header' }, this.isMobileLayout ? mobileNav : desktopNav);
    }
    get el() { return index.getElement(this); }
};
...
const CcdesignIcon = class {
    ...
    render() {
        ...
        return index.h("div", { class: `icon ${inputSize} ${inputColor}` });
    }
    get iconEl() { return index.getElement(this); }
...
const CcdesignLazyImage = class {
     ...
     */
    render() {
        return index.h("img", { class: this.classNames, "data-src": this.imgSrc, alt: this.alt });
    }
    get el() { return index.getElement(this); }
};
...
const CcdesignSocialContainer = class {
     ...
     */
    render() {
        return (index.h("div", { class: 'circle circle--outercircle' }, index.h("div", { class: 'circle circle--innercircle' }, index.h("ccdesign-button", { icon: this.icon, color: 'blue', url: this.url, alt: this.icon }))));
    }
};
...
const MobileNav = class {
     ...
     */
    render() {
        const logo = (index.h("ccdesign-lazy-image", { "img-src": 'assets/favicon.svg', alt: 'CCDesigns' }));
        const openNav = !this.isRootPage ? (index.h("ccdesign-button", { icon: 'bars', type: 'text', color: 'light-grey', onClick: () => this.openNav(), class: `${this.isNavOpen ? 'visible' : ''}`, alt: 'Menu' }, "Menu")) : ('');
        const closeNav = (index.h("ccdesign-button", { type: 'text', icon: 'times', color: 'light-grey', onClick: () => this.closeNav(), alt: 'close' }));
        const backNav = this.isRootPage ? (index.h("ccdesign-button", { icon: 'chevron-left', type: 'text', color: 'light-grey', onClick: () => this.backRootPage(), alt: 'chevron-left' })) : ('');
        const navHeader = (index.h("div", { class: 'navbar__header' }, !this.isRootPage ? openNav : backNav, logo));
        const getNavItems = () => {
            if (this.data === undefined) {
                return;
            }
            ...
            returnItems = navItems.map((item) => (index.h("li", { id: 'mobile' + item.id, class: 'navbar__mobile-item' }, index.h("a", { id: item.id, class: 'navbar__mobile-link', href: item.url }, index.h("div", null, item.name)))));
            // eslint-disable-next-line @stencil/render-returns-host
            return returnItems;
        };
        const getNav = () => index.h("ul", { class: 'navbar__mobile-list' }, getNavItems());
        const navbar = (index.h("nav", { class: `navbar__mobile ${!this.isNavOpen ? 'hide-left' : 'hide-right'}` }, index.h("div", { class: 'navbar__mobile-sub-header' }, closeNav), [getNav()]));
        const navbarInteraction = index.h("div", { class: 'navbar__mobile--interaction' });
        // eslint-disable-next-line @stencil/render-returns-host
...
exports.ccdesign_button = CcdesignButton;
exports.ccdesign_carousel_new = CcdesignCarouselNew;
exports.ccdesign_copy = CcdesignCopy;
exports.ccdesign_cv_template = CcdesignCvTemplate;
exports.ccdesign_footer = CcdesignFooter;
exports.ccdesign_header = CcdesignHeader;
exports.ccdesign_icon = CcdesignIcon;
exports.ccdesign_lazy_image = CcdesignLazyImage;
exports.ccdesign_placeholder = CcdesignPlaceholder;
exports.ccdesign_separator = CcdesignSeparator;
exports.ccdesign_social_container = CcdesignSocialContainer;
exports.mobile_nav = MobileNav;
